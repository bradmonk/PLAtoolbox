%% PLAtoolbox.m - PLA TOOLBOX

clc; close all; clear;
cd(fileparts(which('PLAtoolbox.m')));
disp('WELCOME TO THE PLA TOOLBOX')
...
[filename, pathname] = uigetfile({'*.j*pg';'*.png';'*.tif*';'*.bmp*'},...
...
%% --- PERFORM CELL AUTOTRACE ---
    ...
    [nbpixels, ~] = cellfun(@size, boundarypixels);
    
    cellpix = boundarypixels;
...
close all
fh1=figure('Units','normalized','Position',[.1 .1 .5 .7],'Color','w');
...
han2 = annotation(gcf,'textbox',...
    [0.1 0.91 0.3 0.05],...
    'Color',[0 .6 0],...
    'String',['Cell Bodies: ', num2str(numel(cellpix))],...
    'FontSize',14,...
    'FontName','Courier',...
    'EdgeColor', 'none',...
    'FitBoxToText','on');

han3 = annotation(gcf,'textbox',...
    [0.1 0.87 0.3 0.05],...
    'Color',[.6 0 0],...
    'String',['PLA (not on cell body): ', num2str(numel(puncpix))],...
    'FontSize',14,...
    'FontName','Courier',...
    'EdgeColor', 'none',...
    'FitBoxToText','on');
...
%{
    % pixels above bwthresh make 1's below make 0's
    bwthresh = .1;
    IMG_BW = im2bw(IMG_filled, bwthresh);


    [L, NUM] = bwlabeln(IMG_BW);
...
imagesc(IMGr);
    title('CLICK-N-DRAG A BOX OVER SOME BACKGROUND THEN CLOSE WINDOW')
h1 = imrect;
...
%% GET FRAME COORDINATES AND CREATE XY MASK
    ...
    mask(pmsk(1):pmsk(2), pmsk(3):pmsk(4)) = 1;
...
text(pmsk(1),(pmsk(3)+pmsk(4))/2,'BACKGROUND','Color','w')
...
[centers, radii] = imfindcircles(im,[15 31], 'Sensitivity',0.9);
